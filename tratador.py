...
def tratar_clientes(
    ...
    emails_corrigidos = (df['email'].str.endswith('.com') &
    ...
    df['dias_desde_cadastro'] = (
        (hoje - df['data_cadastro']).dt.days
    ).where(df['data_cadastro'].notna())
    ...
    df['cpf_cnpj_mascarado'] = df['cpf_cnpj_normalizado'].apply(
        mascarar_cpf_cnpj)
    ...
    relatorio.registro('E-mails inválidos (flagged)',
                       (~df['email_valido']).sum())
    relatorio.registro('Idades calculadas',
                       df['idade'].notna().sum())
...
def tratar_vendas(
    ...
    df['margem_pct'] = (
        df['lucro_bruto'] / df['subtotal'] * 100
    ).where(df['subtotal'] > 0).round(2)
    ...
    relatorio.registro('Linhas de entrada',         linhas_inicial)
    relatorio.registro('Pedidos ativos (saída)',
                       df_ativos['id_pedido'].nunique())
    relatorio.registro('Pedidos cancelados',
    ...
    relatorio.registro('Receita total (ativos)',
                       f"R$ {df_ativos['subtotal'].sum():,.2f}")
    relatorio.registro('Lucro bruto total (ativos)',
                       f"R$ {df_ativos['lucro_bruto'].sum():,.2f}")
...
def tratar_produtos_estoque(
    ...
    # ── Conversão de data ──────────────────────────────────────────────────
    df['ultima_atualizacao'] = pd.to_datetime(
        df['ultima_atualizacao'], errors='coerce')

    # ── Cálculos de margem ─────────────────────────────────────────────────
    df['margem_bruta'] = (df['preco_venda'] - df['preco_custo']).round(2)
    df['margem_pct'] = (
        df['margem_bruta'] / df['preco_venda'] * 100
    ).where(df['preco_venda'] > 0).round(2)
    ...
    relatorio.registro('Produtos únicos',
                       df['id_produto'].nunique())
    relatorio.registro('Sem estoque (qtd = 0)',
                       (df['estoque_qtd'] == 0).sum())
    relatorio.registro('Margem média (%)',
                       f"{df['margem_pct'].mean():.1f}%")
...
def tratar_equipe_lojas(
    ...
    df['tempo_casa_anos'] = (
        delta.dt.days // 365).where(df['data_admissao'].notna())
    df['tempo_casa_meses'] = (
        delta.dt.days // 30).where(df['data_admissao'].notna())
    ...
    relatorio.registro('Salário médio',
                       f"R$ {df['salario'].mean():,.2f}")
    relatorio.registro('Tempo médio de casa (anos)',
                       f"{df['tempo_casa_anos'].mean():.1f}")
...
def main() -> None:
    ...
    tarefas = [
        ...
        (
            ...
            [os.path.join(
                pasta_saida, 'produtos_estoque_tratado.csv'), relatorio],
        ),
        (
            'equipe_lojas.csv',
            tratar_equipe_lojas,
